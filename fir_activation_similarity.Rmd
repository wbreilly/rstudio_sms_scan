...
actsim_func1 = function(region1,region2)
  ...
  temp.df = rsa %>% filter(roi == region1) 
  temp.df = droplevels(temp.df)
  dat1 = temp.df$bold
  
  temp.df = rsa %>% filter(roi == region2) 
  temp.df = droplevels(temp.df)
  dat2 = temp.df$bold
...
df = CartProduct(df,as.vector(allrois$ROI))
df$region2 = df$var2
df$var2 = NULL
...
tstatdat = arrange(tstatdat,region1,region2,roi_group,desc(zval))
