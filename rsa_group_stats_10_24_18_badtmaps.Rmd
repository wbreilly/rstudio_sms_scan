...
rsa.SVDP = read.table("/Users/wbr/walter/fmri/sms_scan_analyses/rsa_singletrial/singletrial_4_rsatoolbox/november_17/RSAmeans_same_verb_10_25_18_5ptile.txt", header = TRUE, stringsAsFactors = TRUE)
...
rsa.SVSS = read.table("/Users/wbr/walter/fmri/sms_scan_analyses/rsa_singletrial/singletrial_4_rsatoolbox/november_17/RSAmeans_same_position_10_25_18_5ptile.txt", header = TRUE, stringsAsFactors = TRUE)
...
```{r}
pm.fig.dat = rsa %>%  filter(roi == "lh-Prec") %>% group_by(sub,condition) %>% summarise(similarity = mean(similarity))
PM.check = ggplot(data = pm.fig.dat, aes(x = factor(condition), y = similarity))  + geom_boxplot(outlier.shape = NA) + scale_y_continuous(limits = c(-.05, .3))
PM.check = PM.check + ylab("Similarity") + theme(text = element_text(size = 30, face = "bold"),axis.title.x=element_blank(),axis.text.x=element_text(angle = -45,hjust = 0)) + scale_x_discrete(labels = c("Intact", "Same Verb","Same Position","Scrambled")) 
PM.check
# ggsave("pm.all.pdf", plot = last_plot(), dpi = 600 )
...
```{r}
rsa.VMPFC = rsa %>% filter(roi == "rh-VMPFC")
...
for (ipage in 1:12) {
  ...
  fname = sprintf("allrois_5ptile_10_24_18_%d.pdf", ipage) 
  ggsave(fname)
}
```
...
```{r}
rsa.mpfc = rsa %>% filter(rsa$roi == "rh-inf-opercular" )
rsa.mpfc = rsa.mpfc %>% group_by(sub,roi,condition) %>% summarise(similarity = mean(similarity))
m1.all = aov_ez("sub", "similarity", rsa.mpfc, within = c("condition"))
m1.all
ls.all = lsmeans(m1.all,~condition,contr = "pairwise", adjust = NULL)
