...
clust.fig.dat = rsa  %>% filter(position %in% 1:10) %>% group_by(sub,condition,roi_group,runpair) %>% summarise(similarity = mean(similarity))
clusters.rois = ggplot(data = clust.fig.dat, aes(x = factor(condition), y = similarity))  + geom_boxplot(outlier.shape = NA)  +  facet_wrap( ~roi_group+runpair) + scale_y_continuous(limits = c(-.05, .2))
...
PM.line = ggplot(data = line.dat, aes(x = position,group = condition, y = similarity))   + geom_smooth(aes(linetype=condition),method="loess")  + facet_wrap(~runpair)# + scale_y_continuous(limits = c(-.05, .3))
...
```{r}
rsa.PM = rsa %>% filter(roi_group == "AT")
