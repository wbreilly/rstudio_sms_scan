...
clusters.rois = ggplot(data = clust.fig.dat, aes(x = factor(condition), y = similarity))  + geom_boxplot(outlier.shape = NA)  +  facet_wrap( ~roi_group) + scale_y_continuous(limits = c(-.03, .025)) + geom_jitter()

clusters.rois = clusters.rois + ylab("BOLD") + theme(text = element_text(size = 30, face = "bold"),axis.title.x=element_blank(),axis.text.x=element_text(angle = -90,hjust = 0)) + scale_x_discrete(labels = c("Intact","Same Position","Scrambled"))
clusters.rois
```
...
rsa.PM = rsa %>% filter(rsa$roi == region) %>% filter(condition != "random")
...
```{r}
lapply(unique(rsa$sub),indie.sub.plots, region = "near_hipp_head_R")
...
# post_rois = unique(LANGrois$ROI)
# post_rois = PM_labs
# post_rois = c("L_A1_ROI","R_A1_ROI")
check = rsa %>% filter(roi_group == "LANG")
post_rois =unique(check$roi)
# post_rois = unique(ATrois$ROI)
# post_rois = "L_V1_ROI"
...
PM.line = ggplot(data = line.dat, aes(x = position,group = condition, y = similarity, colour = condition)) + geom_line(size = 2) +geom_point(size = 2) # aes(linetype=condition) 
PM.line = PM.line + scale_color_brewer(palette = "Dark2") + theme(text = element_text(size = 30, face = "bold")) + theme(strip.background = element_blank(),strip.text= element_blank()) + theme(legend.position="none") + xlab("timepoint") + ylab("BOLD")
PM.line
...
#+  facet_wrap_paginate(~roi,nrow=3,ncol=2,page = ipage) 
  ...
  fname = sprintf("march19_uni_glass_conplots/%s_timcourse.pdf", post_rois[iroi])
  ggsave(fname, plot = PM.line, dpi = 600,width = 8, height = 5,units = "in")
}
```
...
tstatdat = lapply(1:32,wbr.sliding.ttest.network,winval=0,region="near_hipp_head_R",con1="intact",con2="scrambled")
...
geom_hline(aes(yintercept = 0)) + scale_y_continuous(limits= c(-4.5 , 3.5),breaks = c(-4,-3,-2,-1,0,1,2,3))
  # geom_segment(x = 18,y =-.25,xend= 18,yend = .25, color = "red",size=.75) 

tstat.plot
# ggsave("march19_uni_tstat/tstat_pmmtl.pdf", plot = tstat.plot, dpi = 600,width = 8, height = 5,units = "in")
```
...
# tstat.plot
fname = sprintf("march19_uni_glass_tstatplots/LANG_%s_timcourse.pdf", inputroi)
...
write_csv(brain_movie_dat, "brainmoviedata_univariate_allglass_3_5_19.csv")
...
rsa.diff.ps = rsa.diff.ps %>% filter(position %in% pstart:pend) %>% group_by(sub,condition,roi_group) %>% filter(roi == region, condition != "random") %>% summarise(simmean = mean(similarity))
rsa.diff.ps = rsa.diff.ps %>%  spread(condition, simmean) 
rsa.diff.ps = rsa.diff.ps %>% mutate(psdiffs = intact - scrambled)
rsa.diff.ps = data.frame(rsa.diff.ps)
...
# prange = sprintf('Timepoint %d',pstart)
# positive rt means faster on intact than scrambled 
# positive ps means higher ps for intact 

 
corr.plot = ggplot(data = rsa.diff.ps,(aes(x = psdiffs, y= rtdiffs)))  + geom_point(size = 3) +   geom_smooth(method='lm')
corr.plot = corr.plot  + theme(legend.position="none") + labs(x = "BOLD Difference", y = "Reaction Time Difference") + xlim(min=-.005, max=.005) + geom_hline(yintercept = 0) + geom_vline(xintercept = 0) + ggtitle(paste(pval,rval,"Timepoint", pstart))
...
# ggsave(sprintf('march19_corr_figs/%s_corrplot_pstart%dpend%d.pdf',region,pstart,pend), plot = corr.plot, dpi = 600,width = 8, height = 5.5,units = "in")
}
...
```{r}
wbr.anyps.corrplot(region="near_hipp_head_L",pstart =6,pend=6)
...
wbr.intacteachp.corrplot = function(region,pstart) {
rsa.diff.ps = rsa %>% filter(sub != "s023") 
# 
...
rsa.diff.ps = rsa.diff.ps %>% filter(position %in% (midval-winval):(midval+winval)) %>% group_by(sub,condition,roi_group) %>% filter(roi == region, condition != "random") %>% summarise(simmean = mean(similarity))
rsa.diff.ps = rsa.diff.ps %>%  spread(condition, simmean) 
rsa.diff.ps = rsa.diff.ps %>% mutate(psdiffs = intact - scrambled)
rsa.diff.ps = data.frame(rsa.diff.ps)
...
corrdat = lapply(1:32,wbr.window.corrplot,winval = 0,region="near_hipp_head_L") # 
...
geom_hline(aes(yintercept = 0)) + scale_y_continuous(limits = c(-.3,.6)) 
...
corr.plot
# ggsave("march19_corr_figs/corrplot_win0_univariate_AT_ns.pdf", plot = corr.plot, dpi = 600,width = 8, height = 5,units = "in")
