...
clust.fig.dat = rsa  %>% filter(position %in% 6) %>% filter(roi =="L_V1_ROI") %>% group_by(sub,condition,roi_group) %>% summarise(similarity = mean(similarity))
...
```{r}
lapply(unique(rsa$sub),indie.sub.plots, region = "R_V1_ROI")
...
rsa.diff.ps = rsa.diff.ps %>% filter(position %in% pstart:pend) %>% group_by(sub,condition,roi_group) %>% filter(roi_group == region, condition != "random") %>% summarise(simmean = mean(similarity))
rsa.diff.ps = rsa.diff.ps %>%  spread(condition, simmean) 
rsa.diff.ps = rsa.diff.ps %>% mutate(psdiffs = intact - scrambled)
rsa.diff.ps = data.frame(rsa.diff.ps)
...
corr.plot = corr.plot  + theme(legend.position="none") + labs(x = "BOLD Difference", y = "Reaction Time Difference") + xlim(min=-.01, max=.01) + geom_hline(yintercept = 0) + geom_vline(xintercept = 0) + ggtitle(paste(pval,rval,"Timepoint", pstart))
...
```{r}
wbr.anyps.corrplot(region="LANG",pstart =9,pend=9)
...
corrdat = lapply(1:32,wbr.window.corrplot,winval = 0,region="AT") # 
