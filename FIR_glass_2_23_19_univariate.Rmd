...
clust.fig.dat = rsa  %>% filter(position %in% 1:7) %>% group_by(sub,condition,roi_group) %>% summarise(similarity = mean(similarity))
...
tstatdat = lapply(1:32,wbr.sliding.ttest.network,winval=0,region="PM",con1="intact",con2="scrambled")
...
geom_hline(aes(yintercept = 0)) + scale_y_continuous(limits= c(-4 , 3.5),breaks = c(-3,-2,-1,0,1,2,3))
...
time.diff.plot = ggplot(data = timecourse_dat,(aes(x =timepoint , y= diff,group =sub)))  + geom_point(size = 1) +geom_line(size =1) + theme(text = element_text(size = 30, face = "bold"))  + ylab("diff")  +
geom_hline(aes(yintercept = 0)) + scale_y_continuous(limits= c(-.025 , .025),breaks = c())
  # geom_segment(x = 18,y =-.25,xend= 18,yend = .25, color = "red",size=.75) 
...
```{r}
write_csv(brain_movie_dat, "brainmoviedata_univariate_allglass_2_26_19.csv")

rois = unique(brain_movie_dat$ROI)
write_csv(data.frame(rois),"unique_rois_all.csv")
```
