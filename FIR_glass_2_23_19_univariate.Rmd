...
rsa = rsa %>% group_by(sub,condition,roi,position) %>% summarise(similarity = mean(bold)) %>%  ungroup()
...
clust.fig.dat = rsa  %>% filter(position %in% 20:25) %>% group_by(sub,condition,roi_group) %>% summarise(similarity = mean(similarity))
clusters.rois = ggplot(data = clust.fig.dat, aes(x = factor(condition), y = similarity))  + geom_boxplot(outlier.shape = NA)  +  facet_wrap( ~roi_group) + scale_y_continuous(limits = c(-.025, .025))
...
tstatdat = lapply(1:32,wbr.sliding.ttest.network,winval=2,region="PM",con1="intact",con2="scrambled")
...
geom_hline(aes(yintercept = 0)) + scale_y_continuous(limits= c(-2.5 , 3.5),breaks = c(-2,-1,0,1,2,3))
