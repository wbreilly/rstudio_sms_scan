...
fconn_func1 = function(subj,con1,region1,region2,run) {
  ...
  temp.df = rsa %>% filter(roi == region1) %>% filter(condition == con1, runnum == run,sub == subj) 
  temp.df = droplevels(temp.df)
  dat1 = temp.df$similarity
  
  temp.df = rsa %>% filter(roi == region2) %>% filter(condition == con1, runnum == run,sub == subj) 
  temp.df = droplevels(temp.df)
  ...
  if (length(dat1) == length(dat2)  && length(dat1) > 0){
    ...
    out = c(pval,unname(rval),subj,con1)
    return(out)
  } else {
...
```{r}
# tstatdat = lapply(unique(rsa$sub),fconn_func1,region1="near_hipp_head_L",region2="near_hipp_head_R",con1="intact", run="run1")
# tstatdat = data.frame(matrix(unlist(tstatdat), nrow=27, byrow=T))
# tstatdat = plyr::rename(tstatdat, c("X1"="pval","X2"="rval","X3"="sub","X4"="condition"))

# Vectorizing a function over multiple arguments
df= data.frame(
  subj = as.vector(rep(unique(rsa$sub),2)),
  con1 = c(rep("intact",27),rep("scrambled",27)),
  region1 = rep("near_hipp_head_L",54),
  stringsAsFactors = FALSE
  )


tstatdat = pmap(df,fconn_func1,region2="near_hipp_head_R", run="run1")
tstatdat = data.frame(matrix(unlist(tstatdat), nrow=54, byrow=T))
tstatdat = plyr::rename(tstatdat, c("X1"="pval","X2"="rval","X3"="sub","X4"="condition"))
