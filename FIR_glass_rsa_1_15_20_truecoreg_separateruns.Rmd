---
title: "rsa_fir_glass_separateruns_10_8_19"
author: "WBR"
date: "10/24/2018"
...
# runpair = 1, run1run2, runpair = 2, run1run3, runpair = 3, run2run3
# rsa = read.table("/Users/wbr/walter/fmri/sms_scan_analyses/rsa_singletrial/rsa_fir/RSAmeans_fir_glass_10_7_19_5ptile_atanh_separate_runs.txt", header = TRUE, stringsAsFactors = TRUE)
rsa = read.table("/Users/wbr/walter/fmri/sms_scan_analyses/rsa_singletrial/rsa_fir/RSAmeans_fir_glass_2_26_20_5ptile_atanh_separate_runs_difseq.txt", header = TRUE, stringsAsFactors = TRUE)
...
# 2_19_20 moved BA44 from FPN to LANG because it's in Broca's area
...
FPNrois = modulelabs %>%  filter(network == 8)
...
rsa$runpair = as.factor(rsa$runpair)
# rsa$position = as.factor(rsa$position)
rsa$roi = as.factor(rsa$roi)
rsa$LEFT = grepl("^L_",rsa$roi)
rsa$DMN = ifelse(rsa$roi_group == "PM","DMN",ifelse(rsa$roi_group == "MPFC","DMN",ifelse(rsa$roi_group =="LAT","DMN","notDMN")))
```

```{r}
# these dfs were run through identical first chunk for roi_group creation etc above. Difference is the txt from matlab with either same schema or different schema-same condition and position rsa 
difseq=read_csv("2_27_20_fir_rsa_difseq_df.csv")
sameseq=read_csv("2_27_20_fir_rsa_sameseq_df.csv")

difseq$schema = "diff"
sameseq$schema = "same"

rsa = rbind(difseq,sameseq)
```

```{r}
#3_5_20 no behavior effect + 1 noisy sub drop
# bad_subs = c("s033", "s019","s025", "s034","s002", "s030","s022","s038","s029")
...
# separate conditions learning acc
...
# rsa = read.table("/Users/wbr/walter/fmri/sms_scan_analyses/rsa_singletrial/rsa_fir/RSAmeans_fir_FS_2_28_19_5ptile_atanh.txt", header = TRUE, stringsAsFactors = TRUE)
# diff schema
rsa = read.table("/Users/wbr/walter/fmri/sms_scan_analyses/rsa_singletrial/rsa_fir/RSAmeans_fir_HIPP_3_11_20_5ptile_atanh_separate_runs_difseq.txt", header = TRUE, stringsAsFactors = TRUE)

# collapse across runpairs
rsa= rsa %>% group_by(sub,roi,condition,position) %>% summarise(similarity = mean(na.omit(similarity)))
...
# these dfs were run through identical first chunk for roi_group creation etc above. Difference is the txt from matlab with either same schema or different schema-same condition and position rsa 
difseq=read_csv("rsa_FShipp_diffschema_3_12_20.csv")
sameseq=read_csv("rsa_FShipp_sameschema_3_12_20.csv")


# which(is.na(difseq$similarity))
# 
difseq$schema = "diff"
sameseq$schema = "same"
# 
...
clust.fig.dat = rsa  %>% filter(position %in% 9:25) %>% group_by(sub,condition,roi_group,schema) %>% summarise(similarity = mean(similarity))
clusters.rois = ggplot(data = clust.fig.dat, aes(x = factor(condition), y = similarity))  + geom_boxplot(outlier.shape = NA)  +  facet_wrap( ~roi_group+schema) + scale_y_continuous(limits = c(-.05, .2))
...
line.dat = rsa.PM  %>% group_by(condition,position,runpair) %>% summarise(similarity = mean(similarity))


PM.line = ggplot(data = line.dat, aes(x = position,group = condition, y = similarity)) + geom_line(aes(linetype=condition)) + facet_wrap(~runpair) 
...
### average within network, poster quality line plots
```{r}
...
line.dat = rsa %>%  filter(roi_group == post_rois[iroi])  %>%  group_by(condition,position,schema) %>% summarise(similarity = mean(similarity))
# line.dat = rsa %>%  filter(DMN == post_rois[iroi])  %>%  group_by(condition,position,runpair) %>% summarise(similarity = mean(similarity)) 


PM.line = ggplot(data = line.dat, aes(x = position,group = schema, y = similarity, colour = schema)) + geom_line(size = 1.5) +geom_point(size = 1.5) +facet_wrap(~condition)# aes(linetype=condition) 
PM.line = PM.line + scale_color_brewer(palette = "Dark2") + theme(text = element_text(size = 15, face = "bold"))  + theme(legend.position="top") + xlab("timepoint") # + theme(strip.background = element_blank(),strip.text= element_blank())
PM.line
...
#+  facet_wrap_paginate(~roi,nrow=3,ncol=2,page = ipage) 
  ...
  fname = sprintf("feb20plots_reprise/%s_FSHIPP_rsa_timcourse_win0_diffschema_3bad.pdf", post_rois[iroi])
...
### poster quality line plots, loop through rois in a network, not tstat
```{r}
rsasubgroup = rsa %>% filter(rsa$roi_group == "FPN")
post_rois = unique(rsasubgroup$roi)
# post_rois = c("L_PSL_ROI","L_55b_ROI")


for (iroi in 1:length(post_rois)) {
# rsa.PM = rsa %>% filter(rsa$frontal == TRUE)
line.dat = rsa %>%  filter(roi == post_rois[iroi]) %>% filter(condition != "random") %>%  group_by(condition,position,runpair) %>% summarise(similarity = mean(similarity)) 


PM.line = ggplot(data = line.dat, aes(x = position,group = condition, y = similarity, colour = condition)) + geom_line(size = 2) +geom_point(size = 2) +facet_wrap(~runpair)# aes(linetype=condition) 
...
#+  facet_wrap_paginate(~roi,nrow=3,ncol=2,page = ipage) 
  ...
  fname = sprintf("oct19_sep_runpairs_plots/FPN_%s_timcourse_win0.pdf", post_rois[iroi])
  ggsave(fname, plot = PM.line, dpi = 600,width = 8, height = 5,units = "in")
}
```
...
### check roi_group and runcompare
```{r}
wbr.sliding.ttest.network = function(midval,region,winval,con1,con2,runcompare) {
# grab grouped/meaned data for each condition 
  # %>% filter(runpair == runcompare)
  # ,LEFT==TRUE
temp.df = rsa %>% filter(roi_group == region,schema=="same")  %>%  filter(condition == con1) %>%   filter(position %in% (midval-winval):(midval+winval)) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
temp.df = droplevels(temp.df)
dat1 = temp.df$similarity

temp.df = rsa %>% filter(roi_group == region,schema=="diff")  %>% filter(condition == con2) %>%   filter(position %in% (midval-winval):(midval+winval)) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
temp.df = droplevels(temp.df)
...
post_rois = unique(rsa$roi_group)
# post_rois = c("PM", "HIPP","AT","IFG")
# post_rois = "HIPP"

for (iroi in 1:length(post_rois)) {
tstatdat = lapply(1:32,wbr.sliding.ttest.network,winval=0,region=post_rois[iroi],con1="random",con2="random",runcompare ="3")
...
geom_hline(aes(yintercept = 0)) + scale_y_continuous(limits= c(-3 , 4),breaks = c(-3,-2,-1,0,1,2,3))
  # geom_segment(x = 18,y =-.25,xend= 18,yend = .25, color = "red",size=.75) 

tstat.plot
fname = sprintf("feb20plots_reprise/%s_FSHIPP_BILAT_rsa_tstat_win0_allrunpair_random_diffschema_3bad.pdf", post_rois[iroi])
...
#############################
same two chunks as immedidately above for network level tstat plots, modified to create unique figure for each roi
```{r}
...
```{r}
# use this to get the true tstats for every timepoint. 
rsanet = rsa %>% filter(roi_group == "FPN")
post_rois = unique(rsanet$roi)
...
#plot that shit
tstat.plot = ggplot(data = tstatdat,(aes(x =timepoint , y= tval,group =1)))  + geom_point(size = 3) +geom_line(size =2) + theme(text = element_text(size = 30, face = "bold"))  + ylab("t stat")  +
geom_hline(aes(yintercept = 0)) + scale_y_continuous(limits= c(-3 , 4),breaks = c(-3,-2,-1,0,1,2,3,4))
  # geom_segment(x = 18,y =-.25,xend= 18,yend = .25, color = "red",size=.75) 

tstat.plot
fname = sprintf("feb20plots/%s_FPNnet_tstat_rsa_win0_allrunpair.pdf", post_rois[iroi])
ggsave(fname, plot = tstat.plot, dpi = 600,width = 8, height = 5,units = "in")
# ggsave("jan20plots/sliding.tstat_other.pdf", plot = tstat.plot, dpi = 600,width = 8, height = 5,units = "in")
}
```

###### Dif schema tstat plots!
### check roi_group and runcompare
```{r}
wbr.sliding.ttest.network.schema = function(midval,region,winval,con1,con2,runcompare) {
# grab grouped/meaned data for each condition 
  # %>% filter(runpair == runcompare)
  # ,LEFT==TRUE
temp.df = rsa %>% filter(roi_group == region) %>% filter(schema == "same") %>%  filter(condition == con1) %>%   filter(position %in% (midval-winval):(midval+winval)) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
temp.df = droplevels(temp.df)
dat1 = temp.df$similarity

temp.df = rsa %>% filter(roi_group == region) %>% filter(schema == "diff") %>% filter(condition == con2) %>%   filter(position %in% (midval-winval):(midval+winval)) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
temp.df = droplevels(temp.df)
dat2 = temp.df$similarity
...
for (iroi in 1:length(post_rois)) {
tstatdat = lapply(1:32,wbr.sliding.ttest.network.schema,winval=0,region=post_rois[iroi],con1="scrambled",con2="scrambled",runcompare ="3")
tstatdat = data.frame(matrix(unlist(tstatdat), nrow=32, byrow=T))
tstatdat$timepoint = 1:32
tstatdat = plyr::rename(tstatdat, c("X1"="pval","X2"="tval"))
...
tstat.plot
fname = sprintf("feb20plots_reprise/%s_BILAT_rsa_tstat_win0_scrambles_diffschema_3bad.pdf", post_rois[iroi])
ggsave(fname, plot = tstat.plot, dpi = 600,width = 8, height = 5,units = "in")
# ggsave("jan20plots/sliding.tstat_other.pdf", plot = tstat.plot, dpi = 600,width = 8, height = 5,units = "in")
}
```
...
```{r}
tempdf = rsa %>% filter(roi_group == "PM")
...
# grab grouped/meaned data for each condition 
temp.df = rsa %>% filter(roi_group == network) %>% filter(condition == con1) %>%   filter(position %in% (midval-winval):(midval+winval)) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
temp.df = droplevels(temp.df)
dat1 = temp.df$similarity
...
list_of_dfs = NULL
list_of_dfs = parLapply(cl,1:32,make.sliding.df,winval=0,network="PM",con1="intact",con2="scrambled")
...
ggsave(sprintf('feb20plots/%s_I-S_corrplot_pstart%dpend%d_runpair%d.pdf',region,pstart,pend,runcompare), plot = corr.plot, dpi = 600,width = 8, height = 5.5,units = "in")
}
...
```{r}
wbr.anyps.corrplot(region="L_10r_ROI",pstart=29,pend=29,runcompare=1)
...
# change back for rt diff # rsa.diff.ps = rsa %>% filter(sub != "s023") 
  rsa.diff.ps = rsa 
# change back roi to roi_group for network analyses
#  filter(runpair == curpair) %>%
rsa.diff.ps = rsa.diff.ps %>% filter(position %in% (midval-winval):(midval+winval)) %>%  filter(roi_group == region, condition != "random") %>% group_by(sub,condition)  %>% summarise(simmean = mean(similarity))
rsa.diff.ps = rsa.diff.ps %>%  spread(condition, simmean) 
rsa.diff.ps = rsa.diff.ps %>% mutate(psdiffs = intact - scrambled)
rsa.diff.ps = data.frame(rsa.diff.ps)

# rsa.diff.ps$rtdiffs = rt.diffs$rtdiff
rsa.diff.ps$rtdiffs = learning.acc$acc
...
corrdat = parLapply(cl,1:32,wbr.window.corrplot,winval = 0,region=post_rois[iroi]) # ,curpair="1"
...
geom_hline(aes(yintercept = 0)) + scale_y_continuous(limits = c(-.9,.9)) 
  # geom_segment(x = 2,y =-.05,xend= 2,yend = .05, color = "red",size=.75) +
  # geom_segment(x = 3,y =-.05,xend= 3,yend = .05, color = "red",size=.75) +

corr.plot

fname = sprintf("march20_corr_search/%s_FSHIPP_slidingcorrplot_rsa_Learningacc_allrunpair.pdf", post_rois[iroi])
ggsave(fname, plot = corr.plot, dpi = 600,width = 8, height = 5,units = "in")
}
```

## diff schema correlation with learning
```{r}
da_ps = c()
# ,curpair
wbr.window.corrplot.schema = function(midval,region,winval) {
# change back for rt diff # rsa.diff.ps = rsa %>% filter(sub != "s023") 
  rsa.diff.ps = rsa 
# change back roi to roi_group for network analyses
#  filter(runpair == curpair) %>%
rsa.diff.ps = rsa.diff.ps %>% filter(position %in% (midval-winval):(midval+winval)) %>%  filter(roi_group == region, condition == "random") %>% group_by(sub,schema)  %>% summarise(simmean = mean(na.omit(similarity)))
rsa.diff.ps = rsa.diff.ps %>%  spread(schema, simmean) 
rsa.diff.ps = rsa.diff.ps %>% mutate(psdiffs = same - diff)
rsa.diff.ps = data.frame(rsa.diff.ps)
...
# changed for PS not diff! 
# fromcor = cor.test(rsa.diff.ps$rtdiffs,rsa.diff.ps$psdiffs)
fromcor = cor.test(rsa.diff.ps$rtdiffs,rsa.diff.ps$same)
...
corrdat = parLapply(cl,1:32,wbr.window.corrplot.schema,winval = 0,region=post_rois[iroi]) # ,curpair="1"
corrdat = data.frame(matrix(unlist(corrdat), nrow=32, byrow=T))
corrdat$timepoint = 1:32
corrdat = plyr::rename(corrdat, c("X1"="pval","X2"="rval"))
...
fname = sprintf("march20_corr_search/%s_FSHIPP_corrplot_rsa_intactlearningacc_allrunpair_randomsame.pdf", post_rois[iroi])
ggsave(fname, plot = corr.plot, dpi = 600,width = 8, height = 5,units = "in")
}
```
