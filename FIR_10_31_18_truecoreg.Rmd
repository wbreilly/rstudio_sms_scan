...
clust.fig.dat = rsa %>% filter(roi =="lh-ANG") %>% filter(condition != "random")  %>% filter(position %in% 1:25)  %>% group_by(sub,condition,position) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
...
```{r}
tstatdat = lapply(1:32,wbr.sliding.ttest,winval=2,region="lh-MTG",con1="intact",con2="scrambled")
