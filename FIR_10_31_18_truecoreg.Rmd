...
rsa = read.table("/Users/wbr/walter/fmri/sms_scan_analyses/rsa_singletrial/rsa_fir/RSAmeans_fir_10_31_18_5ptile.txt", header = TRUE, stringsAsFactors = TRUE)
...
post_rois = c("lh-Prec","rh-Prec", "lh-occ-pole", "rh-occ-pole","lh-MTG","lh-inf-triang","lh-inf-opercular","lh-inf-orbital","rh-MPFC","rh-ANG","lh-ANG","lh-phc-ant","rh-phc-ant")
...
#+  facet_wrap_paginate(~roi,nrow=3,ncol=2,page = ipage) 
  ...
  fname = sprintf("sfn18_figs/%s_timcourse.pdf", post_rois[iroi])
...
clust.fig.dat = rsa %>% filter(roi =="lh-Prec") %>% filter(condition != "random")  %>% filter(position %in% 1:25)  %>% group_by(sub,condition,position) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
clust.fig.dat = droplevels(clust.fig.dat)
# clust.fig.dat$roi = as.factor(clust.fig.dat$roi)
a1 = aov_ez("sub", "similarity", clust.fig.dat, within = c("condition","position"))
a1
em1 = emmeans(a1, ~ condition)
pairs(em1, adjust = NULL)
# contrast(ls.m1, alpha=0.05, method="pairwise", adjust= "holm")


# only intact and scrambled and positions 1:25
  # lh-inf-opercular trending main effect of condition (p = .08), main effect of position (p < .0001), no interaction
  # lh-inf-triang, main effect of position, marginal interaction
  # lh-inf-orbital, main effect of condition (p = .04), main effect of position (p < .0001), no interaction  
  # lh-Prec main effect of position (p < .0001) and condition*position interaction (p =.002)
  # rh-Prec main effect of position (p < .0001) and  trending condition*position interaction (p =.07)
  # rh-MPFC main effect of condition p = .04, main effect of position, no interaction
  # lh-occ-pole only main effect of position
  # rh-occpole main effect of position and trending interaction...p = .06. top down prediction ??
  # lh-ANG has marginal condition*position interaction p = .11
  
```

```{r}
clust.fig.dat = rsa.PMAT %>% filter(roi =="lh-Prec") %>% filter(condition == "intact") %>%   filter(position == 1 |  position == 2 |position == 3 | position == 4 | position == 5) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
clust.fig.dat = droplevels(clust.fig.dat)
intactdat = clust.fig.dat$similarity

clust.fig.dat = rsa.PMAT %>% filter(roi =="lh-Prec") %>% filter(condition == "scrambled") %>%   filter(position == 1 |  position == 2 |position == 3 | position == 4 | position == 5) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
clust.fig.dat = droplevels(clust.fig.dat)
scrambleddat = clust.fig.dat$similarity

t.test(intactdat, scrambleddat, paired = TRUE)
...
wbr.first5ps.ttest = function(region,df,con1,con2) {
  temp.df = df %>% filter(roi == region) %>% filter(condition == con1) %>%   filter(position == 1 |  position == 2 |position == 3 | position == 4 | position == 5) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
temp.df = droplevels(temp.df)
dat1 = temp.df$similarity

temp.df = df %>% filter(roi == region) %>% filter(condition == con2) %>%   filter(position == 1 |  position == 2 |position == 3 | position == 4 | position == 5) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
temp.df = droplevels(temp.df)
...
```{r}
# roi.list = list(unique(rsa.PM$roi))
# for (iroi in PM_rois){
# wbr.first5ps.ttest(rsa,,'intact','scrambled')
# }

the_rois = c('lh-Prec', 'rh-Prec')

lapply(HIPP_labs, wbr.first5ps.ttest, df=rsa,con1='intact',con2='scrambled')

```

# sliding window ttests!
################################
```{r}
...
```{r}
tstatdat = lapply(1:32,wbr.sliding.ttest,winval=2,region="rh-hipp-body",con1="intact",con2="scrambled")
...
lapply(HIPP_labs, wbr.allps.ttest, df=rsa,con1='intact',con2='random')
...
```{r}
wbr.last5ps.ttest = function(region,df,con1,con2) {
  temp.df = df %>% filter(roi == region) %>% filter(condition == con1) %>%   filter(position == 21 |  position == 22 |position == 23 | position == 24 | position == 25) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
temp.df = droplevels(temp.df)
dat1 = temp.df$similarity

temp.df = df %>% filter(roi == region) %>% filter(condition == con2) %>%   filter(position == 21 |  position == 22 |position == 23 | position == 24 | position == 25) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
temp.df = droplevels(temp.df)
dat2 = temp.df$similarity
if (length(dat1) == length(dat2)){
  print(region)
...
lapply(HIPP_labs, wbr.last5ps.ttest, df=rsa,con1='intact',con2='random')

# FOR LAST 5 PS
#left inf operc P = .04
# left inf trian p = .05
# rh-MPFC is not sig
# HIpp had a couple ps = .10
# pm has nada
#  at has nada

# intact > random in right hipp head p = .03 
