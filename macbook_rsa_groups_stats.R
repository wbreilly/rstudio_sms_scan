...
rsa.SVDP = read.csv("RSAmeans_FS_ANTS_SVDPforrandom_allps_pears_1.15betathresh_3_3_18.csv", header = TRUE, stringsAsFactors = TRUE)
...
rsa.SVSS = read.csv("RSAmeans_FS_ANTS_SSsamepos_allps_pears_1.15betathresh_3_3_18.csv", header = TRUE, stringsAsFactors = TRUE)
...
# AT.plot
AT.plot = ggplot(data = rsa.AT, aes(x = factor(condition), y = similarity)) +coord_cartesian(ylim=c(-.35,.45))+ geom_boxplot() + facet_wrap(~roi)
AT.plot
...
HIPP.plot = ggplot(data = rsa.HIPP, aes(x = factor(condition), y = similarity)) +coord_cartesian(ylim=c(-.25,.3))+ geom_boxplot() + facet_wrap(~roi)
HIPP.plot
##########################
...
# rsa.HIPP.body = rsa %>%  filter(roi == "rh-hipp-body")
...
# look in left AT 
# left  only 
rsa.AT.left = rsa.AT %>% filter(roi != "lh-TPole", roi != "rh-TPole")
m1.AT = aov_ez("sub", "similarity", rsa.AT.left, within = c("condition"))
m1.AT
