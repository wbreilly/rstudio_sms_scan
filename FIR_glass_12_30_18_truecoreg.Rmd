...
clust.fig.dat = rsa  %>% filter(position %in% 1:7) %>% group_by(sub,condition,sub_group) %>% summarise(similarity = mean(similarity))
clusters.rois = ggplot(data = clust.fig.dat, aes(x = factor(condition), y = similarity))  + geom_boxplot(outlier.shape = NA)  +  facet_wrap( ~sub_group) + scale_y_continuous(limits = c(-.05, .2))
...
#+  facet_wrap_paginate(~roi,nrow=3,ncol=2,page = ipage) 
  ...
  fname = sprintf("glasser_3network_plots/AT_%s_timcourse.pdf", post_rois[iroi])
...
temp.df = rsa %>% filter(othergroup == region) %>% filter(condition == con1) %>%   filter(position %in% (midval-winval):(midval+winval)) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
temp.df = droplevels(temp.df)
dat1 = temp.df$similarity

temp.df = rsa %>% filter(othergroup == region) %>% filter(condition == con2) %>%   filter(position %in% (midval-winval):(midval+winval)) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
temp.df = droplevels(temp.df)
...
tstatdat = lapply(1:32,wbr.sliding.ttest.network,winval=2,region="PM_super",con1="intact",con2="scrambled")
...
# grab grouped/meaned data for each condition 
temp.df = rsa %>% filter(othergroup == network) %>% filter(condition == con1) %>%   filter(position %in% (midval-winval):(midval+winval)) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
temp.df = droplevels(temp.df)
dat1 = temp.df$similarity

temp.df = rsa %>% filter(othergroup == network) %>% filter(condition == con2) %>%   filter(position %in% (midval-winval):(midval+winval)) %>% group_by(sub) %>% summarise(similarity = mean(similarity)) %>%  ungroup()
dat2 = temp.df$similarity
...
list_of_dfs = NULL
list_of_dfs = parLapply(cl,1:32,make.sliding.df,winval=2,network="PM_super",con1="intact",con2="scrambled")
...
```{r}
split_point = sample(c(4:29),1)
if timepoint < split_point{
  timepoint + 
...
# # network level sliding window ttests with PERMUTATION cluster correction
# ```{r}
# wbr.sliding.ttest.network.perm = function(midval,region,winval,con1,con2) {
# # grab the data   
...
# 
# if (length(dat1) == length(dat2)  && length(dat1) > 0){
#   # permute condition labels by multiplying random vector of 1 and -1 with true difference data
#   labshuffle = sample(c(-1,1),27,replace=TRUE)
#   diff = dat1 - dat2
#   diff = diff*labshuffle
#   # computing tstat this way is identical to paired samples ttest
#   result = t.test(diff)
#   pval = result$p.value
#   tval = result$statistic
#   out = c(pval,tval)
...
# 
# ```{r}
# # put the perm stats together
# get.clust.size = function(permnum){
# print(sprintf('Perm #%d!\n',permnum))
# tstatdat = lapply(1:32,wbr.sliding.ttest.network.perm,winval=2,region="PM",con1="intact",con2="scrambled")
# tstatdat = data.frame(matrix(unlist(tstatdat), nrow=32, byrow=T))
# tstatdat$timepoint = 1:32
# tstatdat = plyr::rename(tstatdat, c("X1"="pval","X2"="tval"))
# 
# # logical of whether each timepoint is significant or not
# smallps = tstatdat$pval < .05
# # magic to find max consecutive TRUEs
# clustsize = max(rle(smallps)$lengths[rle(smallps)$values])
# if (clustsize < 1){
#   clustsize = 0
...
# 
# ```{r}
# clustsizehistdata = unlist(lapply(1:1000,get.clust.size))
```
...
```{r}
tstatdat = lapply(1:32,wbr.sliding.ttest,winval=0,region="L_POS1_ROI",con1="intact",con2="scrambled")
tstatdat = data.frame(matrix(unlist(tstatdat), nrow=32, byrow=T))
tstatdat$timepoint = 1:32
tstatdat = plyr::rename(tstatdat, c("X1"="pval","X2"="tval"))
...
#plot that shit
tstat.plot = ggplot(data = tstatdat,(aes(x =timepoint , y= tval,group =1)))  + geom_point(size = 3) +geom_line(size =2) + theme(text = element_text(size = 30, face = "bold"))  + ylab("t stat")  +
geom_hline(aes(yintercept = 0)) + scale_y_continuous(limits= c(-3 , 3.5),breaks = c(-2,-1,0,1,2,3))
# +
  # geom_segment(x = 18,y =-.25,xend= 18,yend = .25, color = "red",size=.75) 

# tstat.plot
fname = sprintf("glasser_3network_tstat_plots/AT_%s_timcourse.pdf", inputroi)
