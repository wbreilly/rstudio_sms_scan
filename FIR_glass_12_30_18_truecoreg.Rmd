...
# adds node strength
modulelabs = read.table("1_23_18_glasser_pmat_nodestrength.csv",header = TRUE, stringsAsFactors = TRUE, sep =",")
...
topmodulelabs = modulelabspct %>% filter(p_node >= .1)
...
clust.fig.dat = rsa  %>% filter(position %in% 1:6) %>% group_by(sub,condition,roi_group) %>% summarise(similarity = mean(similarity))
...
list_of_dfs = NULL
list_of_dfs = parLapply(cl,1:32,make.sliding.df,winval=2,network="PM",con1="intact",con2="scrambled")
...
permhist = unlist(parLapply(cl,1:1000,run.the.perms))
