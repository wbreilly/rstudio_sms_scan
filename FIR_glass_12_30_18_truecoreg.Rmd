...
rsa = read.table("/Users/wbr/walter/fmri/sms_scan_analyses/rsa_singletrial/rsa_fir/RSAmeans_fir_glass_2_23_19_5ptile.txt", header = TRUE, stringsAsFactors = TRUE)
...
tstatdat = lapply(1:32,wbr.sliding.ttest.network,winval=0,region="PM_super",con1="intact",con2="scrambled")
...
evaltval = function(data){
  temp_timecourse_dat = data
  tvals = unlist(lapply(1:10,ttestfunky,temp_timecourse_dat))
  maxt = max(tvals)
  # mint = min(tvals)
...
```{r}
tstatdat = lapply(1:32,wbr.sliding.ttest,winval=2,region="lh-Prec",con1="intact",con2="scrambled")
tstatdat = data.frame(matrix(unlist(tstatdat), nrow=32, byrow=T))
tstatdat$timepoint = 1:32
tstatdat = plyr::rename(tstatdat, c("X1"="pval","X2"="tval"))
...
# tstat.plot
fname = sprintf("fsants_tstat_plots/%s_timcourse.pdf", inputroi)
...
```{r}
write_csv(brain_movie_dat, "brainmoviedata_rsa_allglass_2_26_19.csv")
